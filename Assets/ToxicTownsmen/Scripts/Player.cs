...
public class Player : MonoBehaviour
{
    ...
    public float attackCooldown = 1f; // Added cooldown variable
    private float cooldownTimer = 0f; // Added cooldown timer
    ...
    void AutoShoot()
    {
        if (cooldownTimer < attackCooldown) return; //if cooldown is active, return.
        ...
        foreach (Collider2D hitCollider in hitColliders)
        {
            if (hitCollider.CompareTag("Enemy"))
            {
                ...
                {
                    attackTimer = 0.0f;
                    mousePos = hitCollider.transform.position;
                    Vector3 dir = transform.position - mousePos;
                    float angle = Mathf.Atan2(dir.y, dir.x) * Mathf.Rad2Deg + 90;
                    transform.rotation = Quaternion.AngleAxis(angle, Vector3.forward);
                    Shoot();
                    cooldownTimer = 0f; // Reset cooldown timer after shooting.
                    break;
                }
            }
        }
